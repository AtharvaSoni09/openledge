    ...
    return (
        <>
            ...
            <div className="container mx-auto px-4 py-12 max-w-6xl">
                ...
                <Head>
                    <title>{bill?.seo_title || bill?.title} | The Daily Law</title>
                    <meta name="description" content={bill?.meta_description || bill?.tldr || ''} />
                    <link rel="canonical" href={`https://thedailylaw.org/legislation-summary/${slug}`} />

                    {/* Open Graph */}
                    <meta property="og:title" content={bill?.seo_title || bill?.title} />
                    <meta property="og:description" content={bill?.meta_description || bill?.tldr || ''} />
                    <meta property="og:type" content="article" />
                    <meta property="og:url" content={`https://thedailylaw.org/legislation-summary/${slug}`} />
                    <meta property="og:image" content={`https://thedailylaw.org/api/og?title=${encodeURIComponent(bill?.seo_title || bill?.title || '')}&billId=${encodeURIComponent(bill?.bill_id || '')}`} />
                    <meta property="og:image:width" content="1200" />
                    <meta property="og:image:height" content="630" />
                    <meta property="og:site_name" content="The Daily Law" />

                    {/* Twitter Card */}
                    <meta name="twitter:card" content="summary_large_image" />
                    <meta name="twitter:title" content={bill?.seo_title || bill?.title} />
                    <meta name="twitter:description" content={bill?.meta_description || bill?.tldr || ''} />
                    <meta name="twitter:image" content={`https://thedailylaw.org/api/og?title=${encodeURIComponent(bill?.seo_title || bill?.title || '')}&billId=${encodeURIComponent(bill?.bill_id || '')}`} />

                    {/* Article specific */}
                    <meta property="article:published_time" content={bill?.created_at} />
                                ...
                                {(() => {
                                    ...
                                    return categories.map((category) => (
                                        <Badge
                                            key={category.id}
                                            variant="outline"
                                            className={`font-sans text-xs ${category.color === 'blue' ? 'border-blue-200 bg-blue-50 text-blue-700' :
